...
edges_inside = cv2.Canny(img, 50, 160, apertureSize = 3)
...
edges_canny = cv2.Canny(thresh1, 50, 160, apertureSize = 3)
...
for i in range(0,NUMBER_OF_PIN):
    for j in range(0, h):
        if(edges_inside_copy[j,cut_index[i]] != 0):
            cropImg = edges_inside_copy[j-5:j+5,cut_index[i]-5:cut_index[i]+5]
            ...
            if(max_val > threshold):
                top_left = (cut_index[i] - 5 + max_loc[0]-10 ,j-5+ max_loc[1]-10)
